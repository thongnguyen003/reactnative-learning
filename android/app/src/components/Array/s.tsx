...
const StudentList23pnv1a = () => {
  ...
  const [isFilteringGrade, setIsFilteringGrade] = useState<boolean>(false);
      ...
      [
        ...
        },
        {
          ...
          onPress: () => {
            ...
            setStudents(updatedStudents);
            Alert.alert('Xóa học sinh thành công'); 
          },
          style: "destructive"
        }
      ],
  ...
  return (
    <View style={styles.container}>
        ...
        <RadioButton.Group
            onValueChange={newValue => setIsFilteringGrade(newValue === 'true')}
            value={isFilteringGrade ? 'true' : 'false'}
        >
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
            <RadioButton value="false" />
            <Text>Không lọc điểm</Text>
            <RadioButton value="true" />
            <Text>Lọc điểm &gt; 8</Text>
            </View>
        </RadioButton.Group>
        <RadioButton.Group
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <View style={{ borderBottomWidth: 1, borderBottomColor: 'gray', paddingVertical: 10 }}>
            <Text>STT: {item.id} Tên: {item.name}</Text>
